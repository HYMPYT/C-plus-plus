...
void save(Film*arr, int size, int id)
{
	FILE*f = fopen("Videostore.save", "wb");
	if (f)
	{
		fwrite(&id, sizeof(int), 1, f);
		fwrite(&size, sizeof(int), 1, f);
		fwrite(arr, sizeof(Film), size, f);
		fclose(f);
	}
}
void load(Film*&arr, int &size, int &id)
{
	FILE*f = fopen("Videostore.save", "rb");
	if (f)
	{
		if (size)
			delete[]arr;
...
int main()
{
	...
	while (n == 1)
	{
		...
		cout << "7 - Save all information." << endl;
		cout << "8 - Download saved information from file." << endl;
		cout << "9 - Exit" << endl << ">>>> ";
		cin >> choose;
		switch (choose)
		{
		...
		case 7:
			system("cls");
			save(videostore, size_mas, id);
			cout << "Information saved successfully." << endl;
			system("pause");
			break;
		case 8:
			system("cls");
			load(videostore, size_mas, id);
			cout << "Information has been successfully uploaded." << endl;
			system("pause");
			break;
		case 9:
			n = 0;
